...
int ERNexp()
{
    int row, col, lc, total_live_cells, ERN;
    cout << "\n Enter row: ";
    cin >> row;
    cout << "\n Enter Column: ";
    cin >> col;
    ...
    do {
        ...
        switch (response)
        {
            case '1':
                ...
                G.~ConwayGame();
                while (!foundpattern && experiment < 200) {
                    experiment++;
                    ConwayGame G(row, col, lc); 
                    total_live_cells = G.CountLiveCells();
                    ERN = row + col + total_live_cells;
                    cout << "\n Experiment #" << experiment << endl;
                    cout << "\n Grid intialized with ERN : " << ERN;
                    G.PrintFrame(G);  // Print the initial frame
                    if (G.checkForBlock()) {
                            cout << "\n Pattern detected in experiment #" << experiment << endl;
                            foundpattern = true;
                            break;
                     }
                    else
            ...
            case '6':
                G.~ConwayGame();
                while (!foundpattern && experiment < 500) {
                    experiment++;
                    ConwayGame G(row, col, lc);  
                    total_live_cells = G.CountLiveCells();
                    ERN = row + col + total_live_cells;
     ...
     do
     {
         ...
         case '2':
             ...
             while (!foundpattern && experimentCount<20) {
                 experimentCount++;
                 ConwayGame GAME(35, 35, 800);  // Create a new game for each experiment

                 cout << "\nExperiment #" << experimentCount << endl;
                 GAME.PrintFrame(GAME);  // Print the initial frame

                 while (GAME.frameno <= 100) {  // Limit frames per experiment
                     GAME.ApplyRulesParallel();
                     GAME.PrintFrame(GAME);  // Print each frame
